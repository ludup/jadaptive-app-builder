...
public abstract class AbstractSearchPage extends TemplatePage implements FormProcessor<SearchForm> {
	...
	public final void processForm(Document document, SearchForm form) throws IOException {
		...
		sortColumn = StringUtils.defaultIfBlank(form.getSortColumn(), form.getSearchColumn());
		setCachedValue("sortColumn", sortColumn);
		
		sortOrder = SortOrder.valueOf(StringUtils.defaultIfBlank(form.getSortOrder(), "ASC"));
	...
	protected void doGenerateTemplateContent(Document document) throws IOException {
		...
		if(Objects.isNull(sortColumn)) {
			sortColumn = getCachedValue("sortColumn", StringUtils.defaultIfBlank(Request.get().getParameter("sortColumn"), template.getDefaultColumn()));
		...
		if(Objects.isNull(order)) {
			order = getCachedValue("sortOrder", StringUtils.defaultIfBlank(Request.get().getParameter("sortOrder"), "ASC"));
