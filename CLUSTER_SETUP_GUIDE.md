...
Create a new file in `conf.d` called `keyserver.properties`. Apply the *same* configuration on all nodes in the cluster.
...
Create a `scheduler.properties` in `conf.d`.  
