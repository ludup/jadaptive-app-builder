...
public class NCSARequestLog implements Closeable
{
    ...
    public void log(HttpServletRequest request, HttpServletResponse response)
    {
        try
        {
            ...
            buf.append(" [");
            buf.append(DateTimeFormatter.ofPattern("dd/MMM/uuuu:HH:mm:ss Z" ). 
            		withZone(ZoneId.systemDefault()).format(getTimestamp(request)));
...
//                buf.append("Async");
            ...
            if (_logDispatch || _logLatency)
            {
                ...
                if (_logDispatch)
                {   
                    long d = getTimestamp(request).toEpochMilli();
                    buf.append(' ');
                    buf.append(now - (d==0 ? getTimestamp(request).toEpochMilli():d));
                }

                if (_logLatency)
                {
                    buf.append(' ');
                    buf.append(now - getTimestamp(request).toEpochMilli());
                }
            }
	...
	private Instant getTimestamp(HttpServletRequest request) {
		return (Instant)request.getAttribute(LoggingInterceptor.TIME_STARTED_ATTR);
	}
     ...
     * @throws IOException
     */
    protected void logExtended(HttpServletRequest request,
    ...
    public void start() throws Exception
    {
        if (_filename != null)
        {
            _fileOut = new FileOutputStream(_filename,_append);
            _closeOut = true;
            LOG.info("Opened " + getFilename());
        }
        else
            _fileOut = System.err;
