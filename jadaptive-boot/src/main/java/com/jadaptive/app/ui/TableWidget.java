		...
		for (FieldTemplate field : page.getTemplate().getFields()) {
			if (field.getViews().contains(FieldView.TABLE)) {
				tr.append(String.format(
						"<th webbits:bundle=\"i18n/%s\" webbits:i18n=\"%s.%s.name\" data-field=\"%s\">[%s]</th>",
						page.getTemplate().getResourceKey(), page.getTemplate().getResourceKey(),
						field.getResourceKey(), field.getResourceKey(), field.getResourceKey()));

			}
		}

		tr.append(
				"<th webbits:bundle=\"i18n\" webbits:i18n=\"default.actions.name\" data-formatter=\"renderActions\"></th>");
