...
public class ChangeMessageText extends AbstractTenantAwareCommand {
	
	@Autowired
	private ApplicationService applicationService; 
	
	public ChangeMessageText() {
		super("change-message-text", "Message Templates",
					UsageHelper.build("change-message-text <shortName> <file>"), 
					"Change the plain text of a message template");
	}
	...
	protected void doRun(String[] args, VirtualConsole console)
		...
		try(InputStream in = file.getInputStream()) {
			message.setPlainText(IOUtils.readStringFromStream(in, "UTF-8"));
		} 
